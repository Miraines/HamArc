...
void ToNumbersI(uint32_t x, uint8_t numbers[], uint8_t byte) {
  for (int i = (byte - 1); i >= 0; --i) {
    ...
    x = x % (static_cast<uint32_t>(pow(256, i))); //получаем остаток для следующего байта
  }
}
...
//Дублируем биты для того, чтобы создать "кодовое слово" для каждого бита
//Это поможет нам быстрее определять произошла ли ошибка
